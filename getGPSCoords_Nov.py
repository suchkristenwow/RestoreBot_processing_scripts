...
measurements = np.genfromtxt("Nov_results/4control/yawHeading_data.csv", delimiter=",", skip_header=1)
compass_data = np.genfromtxt("Nov_results/4control/processed_compass_heading.csv", delimiter=",", skip_header=1)  # t, compassHeading

# Initialize results file if it doesn't exist
'''
if not os.path.exists("Nov_results/results.csv"):
    with open("Nov_results/results.csv", mode='w', newline='') as file:
...
with open("Nov_results/4control/results.csv", mode='w', newline='') as file:
    writer = csv.writer(file)
    ...
    for measurement in measurements:
        ...
        print("cam_range: {}, cam_bearing_rad: {}".format(cam_range, cam_bearing_rad)) 
        cam_lat, cam_lon = calculate_new_gps(lat, lon, cam_range, np.rad2deg(cam_bearing_rad % (2 * np.pi)))

        print("cam_lat: {}, cam_lon: {}".format(cam_lat,cam_lon)) 

        #This is the range and bearing from the camera to each of the corners of the camera
        frame_corners = {
        ...
        print("right_frame_coords: ",right_frame_coords) 


        # Write results
        row = [t, lat, lon, pointer_lat, pointer_lon,

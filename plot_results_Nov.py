...
frame_dir = "/media/kristen/easystore2/RestorebotData/Nov2022/Nov6/4control/_2022-11-06-07-48-37_0/front_camera_frames"  # Directory containing frame images
...
covariance_dir = "Nov_results/4control/covariance_matrices"
...
def find_closest_frame(timestamp):
    ...
    frame_files = [x for x in frame_files if 'adjusted' not in x]
    ...
    if len(matching_files) > 0:
        ...
        if delta_t > 1:
            raise OSError 
            #return find_closest_frame(timestamp - 105218120.01435804)
    else: 
        matching_files = [
            os.path.join(frame_dir, f)
            for f in os.listdir(frame_dir)
        ...
        matching_file_timestamp = int(matching_files[0][:-4] ) 
        delta_t = abs(matching_file_timestamp - timestamp) 
        print("closest_timestamp: ",matching_file_timestamp)
        print("timestamp: ",timestamp)
        print("delta_t: ",delta_t)
        if delta_t > 1: 
            raise OSError 
...
automated_annotations = np.genfromtxt("Nov_results/4control/data.csv", delimiter=",")  # [row_tstep, true_course, measured_yaw, angular_vel]
annotated_tsteps = automated_annotations[:, 0]

# Read the GPS data
gps_results = np.genfromtxt("Nov_results/4control/results.csv", delimiter=",", skip_header=1)
...
# Extract left frame corner coordinates
leftCam_top_right_lat, leftCam_top_right_lon = gps_results[:, 15], gps_results[:, 16]
leftCam_top_left_lat, leftCam_top_left_lon = gps_results[:, 17], gps_results[:, 18]
leftCam_bottom_right_lat, leftCam_bottom_right_lon = gps_results[:, 19], gps_results[:, 20]
leftCam_bottom_left_lat, leftCam_bottom_left_lon = gps_results[:, 21], gps_results[:, 22]

# Extract left camera center
left_cam_lat, left_cam_lon = gps_results[:, 23], gps_results[:, 24]

# Extract right frame corner coordinates
rightCam_top_right_lat, rightCam_top_right_lon = gps_results[:, 25], gps_results[:, 26]
rightCam_top_left_lat, rightCam_top_left_lon = gps_results[:, 27], gps_results[:, 28]
rightCam_bottom_right_lat, rightCam_bottom_right_lon = gps_results[:, 29], gps_results[:, 30]
rightCam_bottom_left_lat, rightCam_bottom_left_lon = gps_results[:, 31], gps_results[:, 32]

# Extract right camera center
right_cam_lat, right_cam_lon = gps_results[:, 33], gps_results[:, 34]
...
left_camera_rectangle = patches.Polygon([[0, 0], [0, 0], [0, 0], [0, 0]], closed=True, edgecolor='green', facecolor='none', linewidth=2, zorder=10)
ax.add_patch(left_camera_rectangle) 

right_camera_rectangle = patches.Polygon([[0, 0], [0, 0], [0, 0], [0, 0]], closed=True, edgecolor='red', facecolor='none', linewidth=2, zorder=10)
ax.add_patch(right_camera_rectangle) 
...
def update(frame):
    ...
    if is_paused:
        ...
        return (recent_plot, older_plot, very_old_plot,
                robot_rectangle, left_camera_rectangle, right_camera_rectangle, 
                ouster_plot)
    ...
    left_camera_coords = [
        [leftCam_top_left_lon[frame], leftCam_top_left_lat[frame]],
        [leftCam_top_right_lon[frame], leftCam_top_right_lat[frame]],
        [leftCam_bottom_right_lon[frame], leftCam_bottom_right_lat[frame]],
        [leftCam_bottom_left_lon[frame], leftCam_bottom_left_lat[frame]]
    ]
    print("left_camera_coords:",left_camera_coords) 
    left_camera_rectangle.set_xy(left_camera_coords)
    

    right_camera_coords = [
        [rightCam_top_left_lon[frame], rightCam_top_left_lat[frame]],
        [rightCam_top_right_lon[frame], rightCam_top_right_lat[frame]],
        [rightCam_bottom_right_lon[frame], rightCam_bottom_right_lat[frame]],
        [rightCam_bottom_left_lon[frame], rightCam_bottom_left_lat[frame]]
    ]
    print("right_camera_coords: ",right_camera_coords) 
    right_camera_rectangle.set_xy(right_camera_coords)
